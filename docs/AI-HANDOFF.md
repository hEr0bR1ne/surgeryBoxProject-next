...
**最后由用户确认烧录在板子上的，是 `hardware_tests/motor_continuous` 常转测试程序，不是仓库根目录的保护主程序。** 用户手动烧录，并提供持续串口输出：`MOTOR_CONTINUOUS: GPIO13=1 GPIO0=0`。

- 常转固件把 D7 恒高、D8 恒低；没有编码器限位、自动停止或串口停止命令。旧电机工具的停止按钮对此程序无效。
- 不能把该固件接上卷线负载后直接恢复24V。恢复联调前，现场确认并换回根目录主程序，电机功率部分先保持不参与联调。
- 当前24V/USB连接、机械位置、控制线是否插回：**未知，需现场确认**。
